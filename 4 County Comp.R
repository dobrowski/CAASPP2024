...
frpm.list <- "Colu|Fres|Imperi|Kern|Lake|Madera|Mendo|Merced|Mont|Tular"
el.list <- "Colu|Imperi|Madera|Merced|Mono|Mont|Beni|Franc|Barb|Stan"
both.list <-  "Colu|Imperi|Madera|Merced|Mont"  # Tulare 
list.20.70 <-  "Colu|Imperi|Madera|Merced|Mont|Tular|Mendo"  # Tulare 
...
caaspp.county %>%
    ...
    labs(x = "",
         y = "",
         color ="",
         title = paste0("CAASPP ", deparse(substitute(ass))," 2023 Rates Meeting or Exceeding Standards "),
         subtitle = desc,
         caption = source.link
    ) + 
        theme(
